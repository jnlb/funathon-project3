...
This section covers how to load a trained segmentation model apply it to a Sentinel-2 image to produce land-cover predictions.

## From MLflow (optional)

If a model was trained in the previous step, this exercice permits to load a model from the MLFlow Model Regestry.
...
Once registered, load the model using `mlflow.pyfunc.load_model`.
...
[https://funathon-2026-project3.lab.sspcloud.fr/docs](https://funathon-2026-project3.lab.sspcloud.fr/docs).
...
**Response:** the S3 filename of the tile containing the point (string).

### `GET /predict_image`

This endpoint runs the full segmentation pipeline on a single Sentinel-2 tile
identified by its S3 path. If a cached prediction exists, it is loaded directly
...
2. Call `GET /predict_image` with `polygons=True`
3. Parse the response as a GeoDataFrame
4. Load the RGB composite with `get_satellite_image()` and display 2 subplots
:::
